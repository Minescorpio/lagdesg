    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-[#1F2937] rounded-lg shadow-sm">
                <form action="{{ route('fournisseurs.update', $fournisseur) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="tel" name="telephone" id="telephone" value="{{ old('telephone', $fournisseur->telephone) }}"
                                class="mt-1 block w-full border border-gray-600 rounded-md shadow-sm py-2 px-3 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                            @error('telephone')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="adresse" id="adresse" rows="3"
                                class="mt-1 block w-full border border-gray-600 rounded-md shadow-sm py-2 px-3 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ old('adresse', $fournisseur->adresse) }}</textarea>
                            @error('adresse')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
